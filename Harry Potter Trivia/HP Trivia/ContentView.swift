...
struct ContentView: View {
    ...
    @State private var movaBackgroundImage = false
        ...
        GeometryReader { geo in
            ZStack {
                Image("hogwarts")
                    ...
                    .offset(x: movaBackgroundImage ? geo.size.width/1.1 : -geo.size.width/1.1)
                    .onAppear{
                        withAnimation(.linear(duration: 60).repeatForever()){
                            movaBackgroundImage.toggle()
                        }
                    }
                VStack {
                    VStack {
                    ...
                    VStack {
                        if animateViewsIn {
                            if store.books.contains(.active) == false {
                                Text("No questions avalable. Go to settings.⬆️")
